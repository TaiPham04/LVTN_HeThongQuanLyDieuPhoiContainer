...
import { useLogCongList, useGhiNhanXuatNhap } from '@/hooks/useLogCong';
import { useBienBanKTDList, useLuuBienBan } from '@/hooks/useBienBanKTD';
import { useChuyenTauList } from '@/hooks/useChuyenTau';
import { useTaiXeList } from '@/hooks/useTaiXe';
import { useContainerLookup } from '@/hooks/useContainer';
...
const loaiktdLabel = { nhapbai: 'Nhập bãi', xuatbai: 'Xuất bãi', thamdinh: 'Thẩm định', haiquan: 'Hải quan' };
const ketluanBadge = (v) => {
  const map = { datieu: { label: 'Đạt tiêu chuẩn', variant: 'success' }, khongdat: { label: 'Không đạt', variant: 'danger' }, tamgiu: { label: 'Tạm giữ', variant: 'warning' } };
...
export default function CongXuatNhapPage() {
  const [tab, setTab] = useState('cong'); // 'cong' | 'bienban'

  return (
    <div>
      <PageHeader
        title="Xuất nhập cổng"
        description="Ghi nhận container vào/ra cổng và biên bản kiểm tra"
      />

      {/* Tab bar */}
      <div style={{ display: 'flex', gap: 4, marginBottom: 20, borderBottom: '2px solid #e5e7eb', paddingBottom: 0 }}>
        {[
          { key: 'cong',    label: 'Cổng vào / ra' },
          { key: 'bienban', label: 'Biên bản kiểm tra' },
        ].map(t => (
          <button
            key={t.key}
            ...
            style={{
              ...
              fontSize: 14, color: tab === t.key ? '#e8920a' : '#6b7280',
              borderBottom: tab === t.key ? '2px solid #e8920a' : '2px solid transparent',
...
function TabBienBan() {
  ...
  const { data, isLoading } = useBienBanKTDList({ trang, search, loaiktd: filterLoai, ketluan: filterKL });
  const luuBienBan          = useLuuBienBan();

  const defaultValues = {
    socontainer: '', loaiktd: 'nhapbai', ketqua_ktd: '',
    bi_hong: false, ketluan: 'datieu',
  ...
  const openModal = () => { reset(defaultValues); setServerErr(''); setModalOpen(true); };
  ...
  const columns = [
    ...
    },
    {
      key: 'loaiktd',
      label: 'Loại kiểm tra',
      render: (v) => <Badge variant="info">{loaiktdLabel[v] || v}</Badge>,
    },
    {
      key: 'ketqua_ktd',
      label: 'Kết quả',
    ...
    },
    {
      ...
      label: 'Hư hỏng',
      align: 'center',
      render: (v) => v ? <Badge variant="danger">Có</Badge> : <Badge variant="success">Không</Badge>,
    },
    {
      key: 'ketluan',
      label: 'Kết luận',
  ...
  return (
    <>
      <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <select value={filterLoai} onChange={e => { setFilterLoai(e.target.value); setTrang(1); }}
          style={{ padding: '8px 12px', border: '1px solid #e2e8f0', borderRadius: 8, fontSize: 13, outline: 'none', background: '#fff' }}>
        ...
        <Button onClick={openModal}>+ Tạo biên bản</Button>
      </div>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Tạo biên bản kiểm tra" width={500}>
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div style={{ marginBottom: 12 }}>
            ...
            <textarea
              rows={3}
              placeholder="Mô tả kết quả kiểm tra chi tiết…"
