...
import { useBienBanKTList, useLuuBienBan } from '@/hooks/nhanvien/useBienBanKT';
...
const loaiBadge = (v) => {
  const map = { nhapbai: 'success', xuatbai: 'warning', thamdinh: 'info', haiquan: 'secondary' };
...
export default function BienBanKTCongPage() {
  ...
  const columns = [
    ...
    { key: 'loaiktd',    label: 'Loại KT',   align: 'center', render: (v) => loaiBadge(v) },
    {
      key: 'ketqua_ktd',
      label: 'Kết quả',
  ...
  return (
    <div>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Tạo biên bản kiểm tra" width={500}>
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
            <div>
              <label style={{ display: 'block', fontSize: 13, fontWeight: 500, color: '#374151', marginBottom: 4 }}>Loại kiểm tra <span style={{ color: '#ef4444' }}>*</span></label>
            ...
            <div>
              <label style={{ display: 'block', fontSize: 13, fontWeight: 500, color: '#374151', marginBottom: 4 }}>Kết luận <span style={{ color: '#ef4444' }}>*</span></label>
          ...
          <div style={{ marginBottom: 12 }}>
            <label style={{ display: 'block', fontSize: 13, fontWeight: 500, color: '#374151', marginBottom: 4 }}>Thời gian kiểm tra <span style={{ color: '#ef4444' }}>*</span></label>
            <input type="datetime-local" style={selectStyle(errors.thoigian_ktd)}
              {...register('thoigian_ktd', { required: 'Thời gian là bắt buộc.' })} />
          </div>
          <div style={{ marginBottom: 12 }}>
            <label style={{ display: 'block', fontSize: 13, fontWeight: 500, color: '#374151', marginBottom: 4 }}>Kết quả kiểm tra <span style={{ color: '#ef4444' }}>*</span></label>
            <textarea rows={4} placeholder="Mô tả chi tiết kết quả kiểm tra…"
      ...
      {/* ── Modal chi tiết ── */}
      {detailRow && (
        <Modal open={!!detailRow} onClose={() => setDetailRow(null)} title={`Biên bản — ${detailRow.socontainer}`} width={480}>
