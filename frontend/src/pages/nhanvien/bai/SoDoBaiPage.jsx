    ...
    <div onClick={() => { if (!isInMode) setPopup(null); }}>
        ...
        >
          ...
          {blockList.map(b => (
            <option key={b.makhuvuc} value={b.makhuvuc}>
              Block {b.tenblock}{b.lablock_lanh ? ' ❄' : ''} ({b.sokhoang}×{b.sohang}×{b.sotang})
            </option>
          ))}
        </select>
        {block && (
          <span style={{ fontSize: 13, color: '#6b7280' }}>
            ...
            {block.lablock_lanh && <span style={{ marginLeft: 10, color: '#0ea5e9', fontWeight: 600 }}>❄ Block lạnh</span>}
          </span>
        )}
      </div>
            ...
            {choGanList.map(c => (
              <div key={c.macontainer}
                style={{
                  ...
                  borderRadius: 8, padding: '7px 12px', background: '#f8fafc', cursor: 'pointer',
                }}
                onClick={() => handleChonGan(c)}
              >
                <div>
                  <div style={{ fontSize: 13, fontWeight: 700, fontFamily: 'monospace', letterSpacing: 1 }}>{c.socontainer}</div>
                  <div style={{ fontSize: 11, color: '#6b7280' }}>{c.mascac ?? '—'} {c.sovoyage ? `· ${c.sovoyage}` : ''} {c.thoigian_vaobai ? `· ${c.thoigian_vaobai}` : ''}</div>
                </div>
            ...
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              {items.map((s, i) => (
                ...
                >
                  ...
                  <span style={{ fontSize: 11, color: '#6b7280', marginLeft: 4 }}>điểm {s.score}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {block && !isLoading && (
        <>
          ...
          {/* Grid */}
          <div style={{ overflowX: 'auto', background: '#fff', border: '1px solid #e2e8f0', borderRadius: 12, padding: '20px 20px 20px 8px' }}>
                    ...
                    >
                      {o.trangthai === 'dangsudung' && o.container && (
                        <>
                          ...
                          <div style={{ fontSize: 9, color: th.text, opacity: 0.8 }}>{o.container.socontainer.slice(4, 7)}</div>
                        </>
                      )}
